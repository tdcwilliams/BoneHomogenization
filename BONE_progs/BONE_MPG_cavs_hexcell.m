function C_eff=BONE_MPG_cavs_hexcell(BDrho,Irr_vars_outer,...
  Irr_vars_inner,N_MP,Nterms);
%% CALL: C_eff=BONE_MPG_cavs_rect_cell(BDrho,Irr_vars_outer,...
%%              Irr_vars_inner,N_MP,Nterms);
%%
%% DESCRIPTION:
...
%  col   = Irr_vars;
   ...
   %%
   %BDrho = .5*[sqrt(3) 2 1];
   BDrho = .5*[1 2/sqrt(3) 1/sqrt(3)];
   if 1%%inner circle
      ecc         = .25;
      crk_fxn     = @CURVEprof_circarc;
      crk_prams   =  {1};%% fraction of circle
      radius      = .2*BDrho(1)
      srt         = {radius*[1 ecc],0,[0 0]};%area=pi*radius^2
   end
