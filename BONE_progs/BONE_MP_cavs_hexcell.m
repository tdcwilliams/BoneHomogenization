function C_eff=BONE_MP_cavs_hexcell(BDrho,Irr_vars,Nterms);
%% CALL: C_eff=BONE_MP_cavs_hexcell(BDrho,Irr_vars,Nterms);
%%
%% DESCRIPTION:
...
%   col    = Irr_vars;
   col   = 'k';
   %%
   BDrho  = .5*[sqrt(3) 2 1];%%[sqrt(3)/2,1,.5]->unit hexagon
   if 1
...
for j=1:Nirregs
   for r=1:Nirregs
      ...
      MP_stuff{1}      = centre(r,:);
      [M_dnW,M_W,xtra] = ...
         BONE_MP_kernelmat_hex_periodic(...
