function C_eff=BONE_MPG_fibres_rect_cell(BDrho,Irr_vars_outer,...
  Irr_vars_inner,N_MP,Nterms);
%% CALL: C_eff=BONE_MPG_fibres_rect_cell(BDrho,Irr_vars_outer,...
%%              Irr_vars_inner,N_MP,Nterms);
%%
%% DESCRIPTION:
...
%   col    = Irr_vars;
   ...
   if 1%%inner circle
     ...
     %%
     BC        = 0;%% BC=0, cts cond; BC=1, debonded/cavity-like condition;
     fibstuff  = {10 1 BC};
   end
   Irr_vars_inner{1} = {crk_fxn,crk_prams,srt,fibstuff};
...
for j=1:Nzones
  geom_stuff   = [GEOM_stuff(j,:),{A,B}];
  %% NB A, B don't matter - can use any GF inside a zone
  %%  - if USE_FS = 0 can use the doubly periodic GF;
  if USE_TN==0
    [MU_dnW,MU_dsW,MU_inner,Fj,...
     MV_dnW,MV_dsW,MV_inner,xtra]   = ...
       BONE_MPG_kernelmat_zone_fibres(...
         ...
         %% THIS IS INDEPENDENT OF MULTIPOLES USED;
  else
    [MU_dnW,MU_dsW,MU_inner,Fj,...
     MV_dnW,MV_dsW,MV_inner,xtra]=...
...
if 0
   load MPfib
%   V2_coeffs=real(VV_coeffs_MP);
%   V1_coeffs=-imag(VV_coeffs_MP);
   tstVeqns = [VV_coeffs_MP,MV*[Wcoeffs_MP_out;UU_coeffs_MP]]
   tstUeqns = [FF,MU*[Wcoeffs_MP_out;UU_coeffs_MP]]
%   [b1Vcoeffs,b1_coeffs]
%   [b2Vcoeffs,b2_coeffs]
end
...
if 0
   ...
   %%
   Hmat(1,1)   = b1_coeffs.'*V1_coeffs/Area;
   Hmat(1,2)   = b1_coeffs.'*V2_coeffs/Area;
   Hmat(2,1)   = b2_coeffs.'*V1_coeffs/Area;
   Hmat(2,2)   = b2_coeffs.'*V2_coeffs/Area;
else
  H0     = [b1_coeffs,b2_coeffs]'*VV_coeffs/Area;
  Hmat   = real(H0*[1i,1]);
end
